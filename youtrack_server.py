...
class YouTrackServer():
    ...
    def init_from_config(self, settings, section):
        ...
        if section.lower().startswith("server/"):
            ...
            self.api = Api(api_token=api_token, youtrack_url=youtrack_url, dbg=self.dbg)
    ...
    def on_suggest(self, user_input: str, items_chain: Sequence):
        ...
        current_suggestion_type: SuggestionMode = self.get_current_suggestionmode(items_chain)
        suggestions = []
        ...
        suggestions.append(self.plugin.create_item(
            ...
            data_bag=kpu.kwargs_encode(url=self.api.create_issues_url(previous_effective_value),effective_value=previous_effective_value)))
    ...
    def get_current_suggestionmode(self, current_items):
    ...
    def add_filter_suggestions(self, actual_user_input, suggestions) -> None:
        api_result_suggestions = self.api.get_intellisense_suggestions(actual_user_input)
